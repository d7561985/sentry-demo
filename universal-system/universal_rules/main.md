...
rule: main
type: core
description: This system is implements a phased development workflow, where each
  mode loads only the rules it needs. The system uses visual Mermaid diagrams and
  selective document loading to optimize context usage.
version: '1.0'
created: '2025-06-28'
modified: '2025-06-28'
dependencies:
...
> **TL;DR:** This system is implements a phased development workflow, where each mode loads only the rules it needs. The system uses visual Mermaid diagrams and selective document loading to optimize context usage.

## 🧭 MODE-SPECIFIC VISUAL MAPS

```mermaid
graph TD
    subgraph Modes["universal development phases"]
...
Refer to [command-execution.md](core/command-execution.md) for detailed guidance. 