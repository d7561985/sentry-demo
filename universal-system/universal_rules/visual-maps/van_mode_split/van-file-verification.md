...
rule: van-file-verification
type: visual-map
description: This system provides a structured approach to verify file structure integrity
  before task implementation, with emphasis on efficient checks and clear status reporting.
version: '1.0'
created: '2025-06-28'
modified: '2025-06-28'
dependencies:
...
$requiredFiles = @(".ai-rules", "tasks.md")
...
5. **One-Pass Verification**: Verify directory structure in a single pass
   ```
   $requiredPaths = @("memory-bank", "docs", "docs\archive", ".ai-rules", "tasks.md")
